...
def test_cron_job_error():
    ...
    add_breadcrumb(
        category="cron",
        message="Starting test cron job",
        level="info"
    )
...
def test_weather_api_error():
    ...
    for i in range(3):
        add_breadcrumb(
            category="http",
            message=f"Weather API call attempt {i+1}",
            level="info"
        )
...
def test_email_notification_error():
    ...
    add_breadcrumb(
        category="email",
        message="Attempting to send migraine alert",
        level="info"
    )
...
def test_performance_issue():
    ...
    capture_message(
        "Slow API response detected: Weather API took 45 seconds to respond",
        level="warning"
    )
    
    set_context("performance_issue", {
...
def test_invalid_llm_response():
    ...
    capture_message(
        "LLM returned invalid probability level: EXTREME (expected: LOW, MEDIUM, HIGH)",
        level="warning"
    )
    
    set_context("invalid_llm_response", {
        "model": "granite-3.0-8b-instruct",
...
def test_high_risk_prediction_alert():
    ...
    capture_message(
        "Generated 5 HIGH risk prediction(s) - unusual spike detected",
        level="info"
    )
    
    set_context("high_risk_alert", {
        "high_risk_count": 5,
        "medium_risk_count": 2,
        "low_risk_count": 1,
        "total_predictions": 8,
