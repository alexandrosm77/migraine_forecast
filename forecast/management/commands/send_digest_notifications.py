            ...
            try:
                ...
                if not force:
                    if not profile.digest_time:
                        self.stdout.write(
                            self.style.WARNING(f'User {user.username} has no digest time set, skipping')
                        )
                        continue
                ...
                migraine_preds = MigrainePrediction.objects.filter(
                    user=user,
                    prediction_time__gte=yesterday,
                    notification_sent=False
                ).select_related('location', 'forecast').order_by('-prediction_time')
                ...
                sinusitis_preds = SinusitisPrediction.objects.filter(
                    user=user,
                    prediction_time__gte=yesterday,
                    notification_sent=False
                ).select_related('location', 'forecast').order_by('-prediction_time')
                ...
                if self.send_digest_email(user, migraine_preds, sinusitis_preds):
                    ...
                    self.stdout.write(
                        self.style.ERROR(f'Failed to send digest to {user.username}')
                    )
                    
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f'Error processing digest for {user.username}: {e}')
                )
                logger.error(f'Error processing digest for {user.username}: {e}', exc_info=True)
        
        self.stdout.write(
            self.style.SUCCESS(f'Digest notification process complete. Sent {digests_sent} digest(s)')
        )
