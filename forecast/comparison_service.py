...
class DataComparisonService:
    ...
    def collect_actual_weather(self, location):
        ...
        try:
            ...
            actual_weather = ActualWeather.objects.create(
                location=location,
                recorded_time=ensure_timezone_aware(
                    datetime.fromisoformat(current.get("time", timezone.now().isoformat()))
                ),
    ...
    def get_forecast_accuracy_metrics(self, location, days=30):
        ...
        reports = WeatherComparisonReport.objects.filter(location=location, created_at__gte=start_date)
