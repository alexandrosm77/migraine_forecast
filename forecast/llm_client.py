...
class LLMClient:
    ...
    def predict_probability(
        ...
        sys_prompt = (
            ...
            "Migraine has several independent weather triggers. Go through every trigger "
            "below, work out the level each one would justify on its own, and answer with the "
            "highest. You must consider all six before answering — a flat pressure reading "
            "does not excuse you from checking the rest.\n\n"
            "1. Pressure. The context gives you the signed change across the window and "
            "against the past 24 hours, each with a severity phrase; judge from those figures "
            "rather than from whether the window looks 'stable', since a steady multi-hour "
            "drift triggers migraine even when each individual hour changes little. The two "
            "comparisons cover different spans, so read each against its own thresholds and "
            "take the worse of the two:\n"
            "   - across the forecast window (a few hours): under 2 hPa -> LOW, 2 to 5 hPa -> "
            "MEDIUM, over 5 hPa -> HIGH.\n"
            "   - against the past 24 hours: under 4 hPa -> LOW, 4 to 8 hPa -> MEDIUM, over "
            "8 hPa -> HIGH.\n"
            "2. Temperature swing versus the past 24 hours, compared against the expected "
            "diurnal range you are given: inside that range -> LOW, exceeding it -> MEDIUM, "
            "roughly double it -> HIGH.\n"
            "3. Air quality, from the band you are given: low -> LOW, moderate -> MEDIUM, "
            "high -> HIGH.\n"
            "4. Absolute heat or cold, which is a trigger in itself regardless of change: "
            "average temperature at or above 32°C, or at or below -5°C -> MEDIUM; at or above "
            "37°C -> HIGH.\n"
            "5. Storms, thunderstorms or an active front inside the window -> at least MEDIUM.\n"
            "6. Humidity below 30% or above 80% -> at least MEDIUM.\n\n"
            "Cautions:\n"
            "- Never lower the level because other triggers look calm. If air quality is banded "
            "high, the answer is HIGH even when pressure is flat, and vice versa.\n"
            "- Temperature change inside the expected diurnal range is not a trigger; ordinary "
            "afternoon warming counts for nothing.\n"
            "- A front that arrives only in the 24-hour outlook, after this window closes, gives "
            "at most MEDIUM. The window being assessed is what matters.\n\n"
            "LOW is the correct and expected answer for a settled window with little pressure "
            "movement, clean air, and no temperature extreme.\n\n"
    ...
    def predict_sinusitis_probability(
        ...
        sys_prompt = (
            ...
            "The dominant drivers are what the air does to the sinus lining: humidity extremes, "
            "irritant particulates, and rapid change. The context bands humidity and each "
            "pollutant for you; judge from those bands rather than from the raw numbers.\n\n"
            "Count how many of these irritant conditions are present:\n"
            "- Humidity flagged as dry air (dries the lining), or as very humid (mold and mite "
            "allergens). A humidity marked comfortable does not count.\n"
            "- Air quality banded moderate or high, especially PM10 and dust, which are coarse "
            "enough to deposit in the sinuses.\n"
            "- Measured pollen banded moderate or above.\n"
            "- Mold risk stated as elevated or high.\n"
            "- Indoor heating likely active, which dries inhaled air further.\n"
            "- Temperature or pressure change beyond the expected diurnal range for the location.\n\n"
            "Then: none of these -> LOW. One -> MEDIUM. Two or more, or any single one at its "
            "high band -> HIGH.\n\n"
            "Important cautions:\n"
            "- Pollen described as 'not measured' or as a calendar season is NOT evidence of "
            "exposure. Never treat a calendar month as a present allergen load.\n"
            "- Do not escalate on a window that is merely stable and unremarkable; stability is "
            "the absence of a trigger, not a trigger.\n"
            "- Do not count the same underlying condition twice under two headings.\n\n"
            "LOW is the correct and expected answer for a settled window with comfortable "
    ...
    def predict_hayfever_probability(
        ...
        sys_prompt = (
            ...
            "The measured pollen counts come with a pre-computed severity band per species "
            "(none, low, moderate, high, very high). The bands are already calibrated for each "
            "species, so trust them rather than judging the raw grains/m³ yourself. 'Max' means "
            "the highest value in the forecast window; it does NOT mean the species is at its "
            "seasonal peak.\n\n"
            "Decide the level from the highest band present:\n"
            "- none or low in every species -> LOW\n"
            "- moderate in at least one species -> MEDIUM\n"
            "- high or very high in at least one species -> HIGH\n\n"
            "Then apply at most one step of adjustment for conditions:\n"
            "- Step down one level when rain is falling in the window, or humidity is very high "
            "with precipitation, since wet air washes pollen out.\n"
            "- Step up one level when the air is dry and warm with strong wind (which disperses "
            "pollen further), when thunderstorms may rupture pollen grains, or when pollution is "
            "genuinely elevated (PM10 above 50 µg/m³ or ozone above 120 µg/m³).\n"
            "- Do not step up on the pollen calendar alone. A species being in season is not "
            "evidence that pollen is airborne now; only the measured counts are.\n"
            "- Do not step up on air quality that is merely ordinary, nor on humidity alone.\n\n"
            "LOW is the correct and expected answer whenever no species reaches its moderate "
            "band. Reserve HIGH for windows where a sufferer would very likely need medication.\n\n"
            "If pollen data is marked unavailable, base the assessment on PM2.5/PM10/ozone/wind/humidity, "
            "do not return HIGH, and explicitly note the reduced confidence.\n"
