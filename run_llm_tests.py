...
def run_test_suite(model_name, base_url, api_key=None, timeout=120):
    ...
    """
    print("=" * 80)
    print("LLM PREDICTION TEST SUITE")
    print("=" * 80)
    print(f"\nModel: {model_name}")
...
def compare_models(result_files):
    ...
    for data in models_data:
        ...
        print(
            f"{model_name:<40} {overall:>6.1%} ({data['summary']['total_correct']}/{data['summary']['total_tests']})    "  # noqa
...
Examples:
  ...
  python run_llm_tests.py --model "gpt-4o-mini" --api-key "sk-..."
    ...
    parser.add_argument(
        "--model", default=None, help="Model name to test (default: use current LLM configuration from database)"
    )
    parser.add_argument(
        "--base-url", default="http://localhost:1234", help="Base URL for LLM API (default: http://localhost:1234)"
    )
    parser.add_argument("--api-key", default=None, help="API key for LLM service (optional for local models)")
    ...
    if args.model is None:
        ...
        print("Using LLM configuration from database:")
