...
def generate_summary_report(results):
    ...
    for i, data in enumerate(results_sorted, 1):
        ...
        # Show sensitivity if not default
        sensitivity = data.get("sensitivity", 1.0)
        if sensitivity != 1.0:
            print(f"       └─ Sensitivity: {sensitivity:.1f}x")
...
def analyze_scenario_performance(results):
    ...
    if results and results[0]["migraine_tests"]:
        ...
        for i in range(num_scenarios):
            ...
            # Get adjusted thresholds if available
            thresholds = results[0]["migraine_tests"][i].get("adjusted_thresholds", {})
            ...
            confidences = [
                r["migraine_tests"][i].get("confidence")
                for r in results
                if r["migraine_tests"][i].get("confidence") is not None
            ]
            ...
            if thresholds:
                print(
                    f"   Thresholds: LOW<{thresholds.get('medium', 0.4):.2f}, MEDIUM:{thresholds.get('medium', 0.4):.2f}-{thresholds.get('high', 0.7)-0.01:.2f}, HIGH≥{thresholds.get('high', 0.7):.2f}"  # noqa
                )
    ...
    if results and results[0]["sinusitis_tests"]:
        ...
        for i in range(num_scenarios):
            ...
            # Get adjusted thresholds if available
            thresholds = results[0]["sinusitis_tests"][i].get("adjusted_thresholds", {})
            ...
            # Calculate average confidence
            confidences = [
                r["sinusitis_tests"][i].get("confidence")
                for r in results
                if r["sinusitis_tests"][i].get("confidence") is not None
            ]
            avg_confidence = sum(confidences) / len(confidences) if confidences else None
            ...
            if thresholds:
                print(
                    f"   Thresholds: LOW<{thresholds.get('medium', 0.35):.2f}, MEDIUM:{thresholds.get('medium', 0.35):.2f}-{thresholds.get('high', 0.65)-0.01:.2f}, HIGH≥{thresholds.get('high', 0.65):.2f}"  # noqa
                )
            print(f"   Accuracy: {accuracy:.1%} ({correct_count}/{total_count} models correct)")
            if avg_confidence is not None:
                print(f"   Avg Confidence: {avg_confidence:.2f}")
...
def analyze_confidence_scores(results):
    ...
    for result in results:
        # Migraine tests
        for test in result.get("migraine_tests", []):
            conf = test.get("confidence")
            if conf is not None:
                if test.get("correct"):
        ...
        # Sinusitis tests
        for test in result.get("sinusitis_tests", []):
            conf = test.get("confidence")
            if conf is not None:
                if test.get("correct"):
                    correct_confidences.append(conf)
    ...
    if correct_confidences or incorrect_confidences:
        ...
        if correct_confidences:
            ...
            print(f"  Correct Predictions: {avg_correct:.3f} avg (n={len(correct_confidences)})")
            print(f"    Min: {min(correct_confidences):.3f} | Max: {max(correct_confidences):.3f}")

        if incorrect_confidences:
            avg_incorrect = sum(incorrect_confidences) / len(incorrect_confidences)
            print(f"  Incorrect Predictions: {avg_incorrect:.3f} avg (n={len(incorrect_confidences)})")
            print(f"    Min: {min(incorrect_confidences):.3f} | Max: {max(incorrect_confidences):.3f}")

        if correct_confidences and incorrect_confidences:
            diff = avg_correct - avg_incorrect
            print(f"\n  Confidence Gap: {diff:+.3f} (correct vs incorrect)")
            if diff > 0.1:
                print("  ✓ Model shows good calibration (more confident when correct)")
            elif diff < -0.1:
                print("  ⚠ Model shows poor calibration (more confident when wrong)")
            else:
                print("  ~ Model confidence is similar for correct and incorrect predictions")
    else:
        print("\nNo confidence scores available in results")
...
def show_failed_predictions_detail(results, verbose=False):
    """Show detailed information about failed predictions."""
    if not verbose:
        return

    print("\n" + "=" * 100)
    print("FAILED PREDICTIONS DETAIL (Verbose Mode)")
    print("=" * 100)
    ...
    for result in results:
        ...
        # Check for failures
        migraine_failures = [t for t in result.get("migraine_tests", []) if not t.get("correct")]
        sinusitis_failures = [t for t in result.get("sinusitis_tests", []) if not t.get("correct")]

        if migraine_failures or sinusitis_failures:
            if not has_failures:
                print(f"\n{model_name}:")
                has_failures = True

            for test in migraine_failures:
                print(f"\n  ✗ Migraine: {test['scenario_name']}")
                print(f"    Expected: {test['expected']} | Predicted: {test['predicted']}")
                print(f"    Weighted Score: {test['weighted_score']:.3f}")
                if test.get("confidence"):
                    print(f"    Confidence: {test['confidence']:.2f}")
                if test.get("rationale"):
                    rationale = test["rationale"][:200] + "..." if len(test["rationale"]) > 200 else test["rationale"]
                    print(f"    Rationale: {rationale}")

            for test in sinusitis_failures:
                print(f"\n  ✗ Sinusitis: {test['scenario_name']}")
                print(f"    Expected: {test['expected']} | Predicted: {test['predicted']}")
                print(f"    Weighted Score: {test['weighted_score']:.3f}")
                if test.get("confidence"):
                    print(f"    Confidence: {test['confidence']:.2f}")
...
Examples:
  ...
  # Show detailed rationale for failed predictions
  python analyze_llm_results.py --verbose llm_results/*.json
        """,
    )

    parser.add_argument("result_files", nargs="+", help="Path(s) to result JSON files")

    parser.add_argument("--verbose", "-v", action="store_true", help="Show detailed rationale for failed predictions")
    ...
    # Show detailed failures if verbose mode
    show_failed_predictions_detail(results, verbose=args.verbose)
